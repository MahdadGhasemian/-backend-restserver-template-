  ...
  constructor() {
      ...
      try {
        ...
        databaseManager.database
          ...
          .toArray(function (err, res) {
            ...
            logger.log_info("get drivers done");
            if (res && res.length > 0) {
    ...
    this.getByIdAndUserId = function (id, userId, callback) {
      try {
        ...
        let o_user_id = new mongodb.ObjectID(userId);
        databaseManager.database.collection("drivers").findOne(
          {
            $and: [
              { _id: o_id },
              { user_id: o_user_id },
          ...
          function (err, res) {
            ...
            logger.log_info("get driver done");
            callback(null, res);
          }
        );
      } catch (error) {
    ...
    this.add = function (newDriver, callback) {
      try {
        if (newDriver.user_id)
          newDriver.user_id = new mongodb.ObjectID(newDriver.user_id);

        databaseManager.database
          .collection("drivers")
          .insertOne(newDriver, function (err, res) {
            if (err) {
              logger.log_error(err);
              return callback(err, null);
            }
            logger.log_info("add driver done");
            callback(null, res);
          });
      } catch (err) {
        logger.log_error(err);
          ...
          .updateOne(myquery, newvalues, function (err, res) {
            try {
              ...
              if (res.result.nModified && res.result.nModified > 0) {
                logger.log_info("update driver done");
                return callback(null, true);
              }
              logger.log_info("no Driver was updated");
              callback(null, false);
            } catch (error) {
              callback(error, null);
            }
      ...
      try {
        databaseManager.database
          ...
          .toArray(function (err, res) {
            ...
            logger.log_info("search driver done");
            callback(null, res);
          });
      } catch (error) {
        logger.log_error(error);
